module tb_rv32i_soc import rv32i_pkg::*; ();
  localparam logic [31:0] EXPECTED_FINAL_PC = 32'h0000_0030;
  localparam int MAX_WAIT_PC = 4096;
  localparam int DRAIN_CYCLES = 24;
  ...
  task automatic check_reg(
    ...
    logic [31:0] actual;
    actual = u_soc.u_cpu.u_register_file.regs_r[reg_idx];
    test_count_r++;
  ...
  task automatic check_mem (
    ...
    logic [7:0] actual;
    actual = u_soc.u_ic.u_dmem.mem_r[addr];
    test_count_r++;
    if (actual === expected) begin
      pass_count_r++;
    end else begin
  ...
  initial begin
    ...
    begin : wait_pc
      int c;
      c = 0;
      while (dbg_pc_w !== EXPECTED_FINAL_PC && c < MAX_WAIT_PC) begin
        @(posedge clk_w);
        c++;
      end
      if (c >= MAX_WAIT_PC) begin
        $fatal(1, "timeout: dbg_pc never reached 0x%08h (last 0x%08h)", EXPECTED_FINAL_PC, dbg_pc_w);
      end
    end
    ...
    check_reg("BEQ skip x9", 9, 32'h0000_0000);
